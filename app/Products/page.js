...
function Products() {
  ...
  let [sortOrder, setSortOrder] = useState("relevance");

  const sortUser = state && state !== "All"
    ? products.filter((e) =>
        e.cat.toLowerCase().includes(state.toLowerCase())
      )
    : products;

  const sortedProducts = sortUser.sort((a, b) => {
    ...
    if (sortOrder === "Latest") {
      return a.role === "latest" ? -1 : 1;
    }
    if (sortOrder === "Oldest") {
      return a.role === "oldest" ? -1 : 1;
    }
    return 0;
  });
  ...
  return (
    <div id="products-page">
      <div className="collection">
        <p id="col">Collection</p>
        <p onClick={() => setState("All")}>All</p>
        <p onClick={() => setState("bags")}>Bags</p>
        <p onClick={() => setState("shirts")}>Shirts</p>
        <p onClick={() => setState("hoodies")}>Hoodies</p>
        <p onClick={() => setState("cap")}>Cap</p>
        <p onClick={() => setState("electronics")}>Electronics</p>
        <p onClick={() => setState("footware")}>Footware</p>
        <p onClick={() => setState("stickers")}>Stickers</p>
        <p onClick={() => setState("drinkware")}>Drinkware</p>
      </div>

      <div className="allProducts">
        {sortedProducts.map((item) => (
          <div
            className="product product-page-pro"
            key={item.id}
      ...
      <div className="sorting">
        <p id="col">Sort by</p>
        <p onClick={() => setSortOrder("relevance")}>Relevance</p>
        <p onClick={() => setSortOrder("Latest")}>Latest</p>
        <p onClick={() => setSortOrder("Oldest")}>Oldest</p>
        <p onClick={() => setSortOrder("Price: Low to High")}>Price: Low to High</p>
        <p onClick={() => setSortOrder("Price: High to Low")}>Price: High to Low</p>
      </div>
    </div>
  );
}
